...
class TerminalPlugin(GObject.Object, Gedit.WindowActivatable):
    ...
    def do_activate(self):
        ...
        self.terminal_id += 1
        bottom.add_item(term, "GeditTerminalPanel" + str(self.terminal_id),
                _("Terminal") + str(self.terminal_id), image)
    ...
    def on_panel_populate_popup(self, panel, menu):
        ...
        item = Gtk.MenuItem.new_with_mnemonic(_("_New Terminal"))
        item.connect("activate", lambda menu_item: self.add_terminal(panel))
        menu.append(item)
        item = Gtk.MenuItem.new_with_mnemonic(_("_Remove Terminal"))
        item.connect("activate", lambda menu_item: self.remove_terminal(panel))
        menu.append(item)
